...
//        CGFloat stringWidth = [self measureTitleAtIndex:self.selectedSegmentIndex].width;
        ...
        //eric:equal width
        CGFloat stringWidth = self.frame.size.width/self.sectionTitles.count -40;
    ...
    if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
        ...
        if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe &&
            ...
            //eric: move selection bar below
            return CGRectMake(x + self.selectionIndicatorEdgeInsets.left, indicatorYOffset+50, sectionWidth - self.selectionIndicatorEdgeInsets.right, self.selectionIndicatorHeight);
        } else {
...
- (void)updateSegmentsRects {
    ...
    if (self.type == HMSegmentedControlTypeText && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
        ...
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            //eric: make equal with
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
//            CGFloat stringWidth = [[UIScreen mainScreen] bounds].size.width/self.sectionTitles.count;
...
-(void)addImageForSegmentIndex:(NSInteger)index
{
    CGRect rectForSelectedIndex;
    CGFloat selectedSegmentOffset = 0;
    if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
    ...
    CGRect rectToAddImage = rectForSelectedIndex;

    
    UIImageView *checkMark = [[UIImageView alloc] initWithFrame:CGRectMake(rectToAddImage.origin.x + rectToAddImage.size.width-20, 0, 20, 20)];
    [checkMark setImage:[UIImage imageNamed:@"checked.png"]];
